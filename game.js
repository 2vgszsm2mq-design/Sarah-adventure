...
function resetPlayer(full=true) {
  Object.assign(player, {
    x:140,y:300,vx:0,vy:0,onGround:false,facing:1,coins:0,letters:0,ammo:3,shield:3,
...
function loadGame() {
  ...
  try {
    ...
    gameState = 'playing'; hideAllOverlays();
    return true;
...
function beginIntro() {
  ...
  gameState = 'intro';
  setTimeout(() => { if (gameState === 'intro') startGameplay(); }, 2600);
}
function startGameplay() { hideAllOverlays(); gameState = 'playing'; saveGame(); }
function newGame() { resetWorld(); resetPlayer(true); heartsShot=[]; particles=[]; beginIntro(); }
...
function update() {
  ...
  if (player.x > 1760) { player.checkpointX = 1820; player.checkpointY = 300; }
  if (player.x > 3250) { player.checkpointX = 3300; player.checkpointY = 300; }

  world.coins.forEach(c => {
    if (!c.collected && Math.hypot((player.x+35)-c.x, (player.y+46)-c.y) < 34) { c.collected = true; player.coins += 1; }
  });
  ...
  if (boss && boss.alive) {
    ...
    if (rectsOverlap(player,boss)) {
      const stomp = player.vy > 2 && player.y + player.h - boss.y < 20;
      if (stomp) hurtPlayer(boss.x + boss.w/2);
...
function drawWorld() {
  ...
  particles.forEach(p => { ctx.fillStyle = `rgba(255,255,255,${p.t/22})`; ctx.beginPath(); ctx.arc(p.x,p.y,30-p.t,0,Math.PI*2); ctx.fill(); });
...
function drawHints() {
  if (gameState !== 'playing') return;
  ctx.fillStyle='rgba(0,0,0,0.35)'; ctx.fillRect(20, canvas.height-160, 420, 84);
