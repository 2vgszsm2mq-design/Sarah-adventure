...
const ui = {
  ...
  victoryFloat: document.getElementById('victoryFloat'),
};

const SAVE_KEY = 'sarah_adventure_save_v7';
let gameState = 'menu';
...
function addImpact(x, y, kind='heart') {
  shakeMag = Math.min(16, shakeMag + (kind === 'boss' ? 9 : 5));
  flashAlpha = Math.min(0.35, flashAlpha + (kind === 'boss' ? 0.18 : 0.08));
  for (let i = 0; i < (kind === 'boss' ? 16 : 10); i++) {
    particles.push({
      x, y,
      vx: (Math.random() - 0.5) * (kind === 'boss' ? 8 : 5),
      vy: (Math.random() - 0.5) * (kind === 'boss' ? 8 : 5) - 1,
      t: kind === 'boss' ? 28 : 22,
      kind: kind === 'boss' ? 'heartburst' : 'sparkle'
    });
  }
}
function showVictoryMoment() {
  ui.victoryFloat.classList.add('visible');
  setTimeout(() => ui.victoryFloat.classList.remove('visible'), 1200);
}
...
function hurtPlayer(sourceX=player.x) {
  ...
  addImpact(player.x + player.w/2, player.y + player.h/2, 'boss');
...
function update() {
  ...
  world.coins.forEach(c => {
    if (!c.collected && Math.hypot((player.x+35)-c.x, (player.y+46)-c.y) < 34) { c.collected = true; player.coins += 1; combo = Math.min(combo + 1, 9); comboUntil = time + 120; setComboPopup(); particles.push({x:c.x,y:c.y,t:18,kind:'spark'}); addImpact(c.x,c.y,'heart'); if (combo >= 5) player.powerTimer = Math.max(player.powerTimer, 180); }
  });
  ...
  heartsShot.forEach(h => {
    ...
    world.enemies.forEach(e => {
      ...
      if (h.x < e.x+e.w && h.x+h.w > e.x && h.y < e.y+e.h && h.y+h.h > e.y) {
        e.alive = false; h.life = 0; particles.push({x:e.x+20,y:e.y+20,t:22}); addImpact(e.x+20,e.y+20,'heart');
      }
    });
    const b = world.boss;
    if (b && b.alive && h.x < b.x+b.w && h.x+h.w > b.x && h.y < b.y+b.h && h.y+h.h > b.y) {
      b.hp -= h.power; h.life = 0; b.x += h.vx > 0 ? 18 : -18; bossFreeze = 6; addImpact(b.x+46,b.y+46,'boss');
      if (b.hp <= 0) { b.alive = false; particles.push({x:b.x+40,y:b.y+40,t:60,kind:'burst'}); showVictoryMoment(); setTimeout(()=>showEnd(true, `SEJR ❤️ Sarah klarede Love Beach med ${player.coins} mønter og ${player.letters} Love Note(s).`), 900); };
    }
  });
  ...
  if (boss && boss.alive) {
    ...
    ui.bossFill.classList.toggle('low', bossPct <= 25);
  } else {
    ui.bossHud.classList.add('hidden');
  }
...
function drawBackground() {
  ...
  const sunX = canvas.width - 180 - (cameraX * 0.08 % 120);
  const sun = ctx.createRadialGradient(sunX, 110, 20, sunX, 110, 90);
  sun.addColorStop(0,'rgba(255,245,180,.95)'); sun.addColorStop(1,'rgba(255,170,150,0)');
  ctx.fillStyle = sun; ctx.fillRect(sunX-110,0,220,220);
  ctx.fillStyle = '#f6b7ce'; ctx.fillRect(0, 520, canvas.width, 220);
  for (let w=0; w<canvas.width+60; w+=60) {
    ctx.fillStyle = `rgba(255,255,255,${0.06 + (w%120===0 ? 0.05 : 0)})`;
    ctx.fillRect(((w - (cameraX*0.55)%60)), 585 + Math.sin((time+w)/18)*2, 42, 4);
  }
...
function drawWorld() {
  ...
  particles.forEach(p => {
    ...
    } else if (p.kind === 'heartburst') {
      const alpha = Math.max(0, p.t/28);
      drawHeart(p.x + p.vx*(28-p.t), p.y + p.vy*(28-p.t), 6, `rgba(255,120,185,${alpha})`);
    } else if (p.kind === 'sparkle') {
      const alpha = Math.max(0, p.t/22);
      ctx.fillStyle = `rgba(255,255,255,${alpha})`;
      ctx.fillRect(p.x + p.vx*(22-p.t), p.y + p.vy*(22-p.t), 4, 4);
...
function render() {
  ctx.save();
  if (shakeMag > 0.2) ctx.translate((Math.random()-0.5)*shakeMag, (Math.random()-0.5)*shakeMag);
  drawBackground();
  drawWorld();
  drawHints();
  ctx.restore();
